...
class DQNAgent(object):
    def __init__(self, gamma, epsilon, lr,input_dims, n_actions, 
        ...
        # create the data for normalization for the state vector before gioing to tensor
        '''
        low_obs = env.get_observation_space().low
        hig_obs = env.get_observation_space().high
        sample_data = np.vstack((low_obs, hig_obs))
        df = pd.DataFrame(sample_data, columns=[f'feature_{i}' for i in range(sample_data.shape[1])])
        # Save to CSV
        df.to_csv('sample_data_addoccupant2_hybrid_flowrate9.csv', index=False)
        '''
        self.sample_data=pd.read_csv('SAC_Q_transformer.csv').to_numpy()
    ...
    def load_models_transfer_learning(self):
        self.q_eval.transfer_learning_load()
        self.q_next.transfer_learning_load()  
